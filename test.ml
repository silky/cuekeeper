...
let singleton = function
  | [x] -> x
  | [] -> assert_failure "Expected one item, but got none"
  | xs -> assert_failure (Printf.sprintf "Expected one item, but got %d" (List.length xs))
...
let rec get_tree rl =
  ...
  |> List.map (fun widget ->
    ...
    let name = Ck_disk_node.name (M.TreeNode.Item.node item) in
